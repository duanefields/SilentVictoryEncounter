...
_.each(cardSlots, function (cardSlot) {
  ...
  var parts = cardSlot.textContent.split(";")

  if (contact.entryName.match(/^(Warship|Capital Ship)/)) {
    contact.entryType = contact.entryName.match(/^Warship/)
      ? "Warship"
      : "Capital Ship"
    var details = parts[33].split("\t")
...
var save = function (contacts, entryType) {
  var records = _.filter(contacts, { entryType })
  var json = JSON.stringify(records, null, 2)
  var fileName = `${entryType.toLowerCase().replace(/\s/g, "")}s.json`
