  ...
  render() {
    ...
    return (
      <div className="text-center">
        ...
        <div>{moment(patrol.currentDate).format("dddd MMMM D, YYYY")}</div>
        ...
        <div className="text-center row">
          <div
            ...
            style={{ height: "5em", marginTop: "1em" }}
          >
            <button
              className="btn btn-primary btn-block"
              onClick={patrol.newPatrol}
            >
              Begin New Patrol
            </button>
          </div>
    ...
    return encounter.contacts.map((c, index2) => {
      ...
      return (
        <tr key={key}>
          ...
          <td className="nowrap">{encounter.travelBox.displayName}</td>
          <td className="nowrap">{encounter.weather.description}</td>
          <td>{c.type}</td>
          <td>
            <If cond={c.tonnage}>
              <span>
