...
export default class Contact {
  ...
  static CreateContacts(encounterType, startDate, patrol) {
    switch (encounterType) {
      ...
      case "Capital Ship":
        return [
          Contact.RandomCapitalShip(),
          Contact.RandomCapitalEscort(startDate),
        ]
      case "Ship+Escort":
        return [
          Contact.RandomFreighter(),
          Contact.RandomMerchantEscort(startDate),
        ]
  ...
  static RandomFreighter() {
    return Contact.RandomShip(Freighters)
  }
  static RandomCapitalShip() {
    return Contact.RandomShip(CapitalShips)
  }
  static RandomWarship() {
    return Contact.RandomShip(Warships)
  }

  static RandomCapitalEscort(startDate) {
    const list =
      startDate.getFullYear() < 1944
        ? CapitalEscortsEarlyWar
        : CapitalEscortsLateWar
  ...
  static RandomMerchantEscort(startDate) {
    const list =
      startDate.getFullYear() < 1944
        ? MerchantEscortsEarlyWar
        : MerchantEscortsLateWar
    return Contact.RandomShip(list, startDate, true)
  }
  ...
  @computed get damageRequired() {
    if (this.entryType === "Capital Ship") return 5
    if (this.tonnage <= 1000) return 1
    if (this.tonnage <= 5000) return 2
    if (this.tonnage <= 9999) return 3
    return 4
  }
