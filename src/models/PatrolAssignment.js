...
export default class PatrolAssignment {
  ...
  static CreateAssignment(base, startDate) {
    ...
    // early or late war?
    var dateRange
    if (startDate.getMonth() < 6) dateRange = `Early ${startDate.getFullYear()}`
    else dateRange = `Late ${startDate.getFullYear()}`
    ...
    console.log(
      `Picking assignment for ${base}, ${dateRange} = ${name}, ${mission}`
    )
